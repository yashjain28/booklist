...
function startup() {
  ...
  function renderBooks(books) {
    ...
    let pastReadCards = generateCards(books.past);
  ...
  function generateCards(reads) {
    let rowOfBooks = [];
    let BOOK_WIDTH = 12 / BOOKS_PER_ROW;
    let output = [];
    for (i in reads) {
      i = +i;
      let b = reads[i];
      let color = getRandomMateralizeColor();
      singleBook = CARD_TEMPLATE.replace(/{{title}}/g, b.title)
      ...
      if ((i + 1) % BOOKS_PER_ROW == 0 || i == reads.length - 1) {
        let row = ROW_TEMPLATE.replace(/{{cards}}/, rowOfBooks.join(""));
        output.push(row);
...
function getDetailed(title, author, callback) {
  ...
  targetURL = GOOGLE_BOOKS_TEMPLATE_URL.replace("{{title}}", titleSafe).replace(
    "{{author}}",
    authorSafe
  );
...
function getRandomMateralizeColor() {
  ...
  var brightness =
    values.brightness[Math.floor(Math.random() * values.brightness.length)];
  var intensity =
    values.intensity[Math.floor(Math.random() * values.intensity.length)];
